...
class UserCredentialsOrm(BaseModelOrm):
    ...
    privileges: Mapped[AuthPrivilegesOrm] = mapped_column(SqlEnum(AuthPrivilegesOrm), nullable=False)
...
class TokenOrm(BaseModelOrm):
    ...
    id: Mapped[str] = mapped_column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(String, ForeignKey('auth.user_credentials.id'))
...
class UserOrm(BaseModelOrm):
    ...
    id: Mapped[str] = mapped_column(String(36), ForeignKey('auth.user_credentials.id'), primary_key=True, index=True)
    ...
    credentials: Mapped[list["UserCredentialsOrm"]] = relationship("UserCredentialsOrm", back_populates="user")
    tokens: Mapped[list["TokenOrm"]] = relationship("TokenOrm", back_populates="user")
    notes: Mapped[list["NoteOrm"]] = relationship("NoteOrm", back_populates="user")
    tasks: Mapped[list["TaskOrm"]] = relationship("TaskOrm", back_populates="user")
    teams: Mapped[list["UserTeamOrm"]] = relationship("UserTeamOrm", back_populates="user")
    statistics: Mapped[list["StatisticOrm"]] = relationship("StatisticOrm", back_populates="user")
    addresses: Mapped[list["AddressOrm"]] = relationship("AddressOrm", back_populates="user")
    calls: Mapped[list["СallOrm"]] = relationship("CallOrm", back_populates="user")
...
class TeamOrm(BaseModelOrm):
    ...
    users: Mapped[list["UserTeamOrm"]] = relationship("UserTeamOrm", back_populates="team")
...
class StatisticOrm(BaseModelOrm):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, index=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(ForeignKey(UserOrm.id, ondelete="CASCADE"))
    period_type: Mapped[StatisticPeriodOrm] = mapped_column(SqlEnum(StatisticPeriodOrm)) 
    ...
    user: Mapped["UserOrm"] = relationship("UserOrm", back_populates="statistics")
...
class AddressOrm(BaseModelOrm):
    ...
    user: Mapped["UserOrm"] = relationship("UserOrm", back_populates="addresses")
...
class СallOrm(BaseModelOrm):
    ...
    transcription: Mapped[str] = mapped_column(String, default="no transcription")
