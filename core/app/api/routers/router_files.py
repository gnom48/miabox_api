...
from app.database import FilesRepository, FileAccessModeOrm
...
@router_files.get("/presigned-url", status_code=status.HTTP_200_OK)
async def pre_signed_file(
    ...
    async with files_repository:
        file_info = await files_repository.get_file_info_by_id(file_id)
        if user_credentials.id in list(map(lambda item: item.user_id, await files_repository.get_file_access(file_id))):
            # FIXME: возвращает ссылку на http://minio:9000 надо заменить на доступный из-вне ip адрес
            # BAD: поврежденный файл
            return minio_client.get_presigned_url(file_info.bucket_name, file_info.obj_name)
        else:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN)
