...
async def task_all(user_credentials: UserCredentials = Depends(get_user_from_request)):
...
async def task_all(user_id: str, user_credentials: UserCredentials = Depends(get_user_from_request)):
    return await Repository.get_all_tasks_by_user_id(user_id, True)


@router_tasks.post("/add", status_code=status.HTTP_201_CREATED)
async def task_add(task: Task, user_credentials: UserCredentials = Depends(get_user_from_request)):
...
async def task_delete(task_id: str, user_credentials: UserCredentials = Depends(get_user_from_request)):
