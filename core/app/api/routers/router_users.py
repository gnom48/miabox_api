...
async def user_edit(user: User, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
...
async def set_image_to_user_by_file(file: UploadFile, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
    try:
        await file.seek(0)
...
async def set_image_to_user(user_id: str, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
