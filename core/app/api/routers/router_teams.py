...
async def team_add(team: Team, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
...
async def team_delete(team_id: str, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
    return await Repository.del_team(team_id)


@router_teams.post("/join")
async def team_join(team_id: str, joined_by: int, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
    try:
...
async def team_leave(team_id: str, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
    return await Repository.leave_team(user_id=user.id, team_id=team_id)


@router_teams.put("/move_team_role")
async def move_team_role(team_id: str, user_id: str, role: UserStatuses, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
    return await Repository.move_team_user_role(user_id=user.id, team_id=team_id, role=role)


@router_teams.get("/my_teams")
async def my_teams(user_credentials: UserCredentials | None = Depends(get_user_from_request)):
