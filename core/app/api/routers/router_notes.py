...
async def notes_all(user_credentials: UserCredentials | None = Depends(get_user_from_request)):
    return await Repository.get_all_notes_by_user_id(user.id)


@router_notes.post("/add", status_code=status.HTTP_201_CREATED)
async def note_add(note: Note, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
...
async def note_delete(note_id: str, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
    return await Repository.del_note(note_id)


@router_notes.put("/edit", status_code=status.HTTP_200_OK)
async def note_edit(note: Note, user_credentials: UserCredentials | None = Depends(get_user_from_request)):
